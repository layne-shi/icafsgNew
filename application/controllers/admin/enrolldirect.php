...
class Enrolldirect extends CI_Controller
{
	protected $tablefunc = 'enroll_direct';
    protected $classname = 'enrolldirect';
	...
	public function add(){
		...
		if($post['action']==site_aurl($this->classname)){
			$data = elements($this->fields,$post['direct']);
			$id=$this->Data_model->addData($data);
            if ($id)
            {
                $this->Enroll_model->saveItems($id,$post['direct']);
            }
    ...
    {
        $where = array('parent' => '0','isdisabled' => 0);
        $groups = $this->Data_model->getData($where,null,null,null,'enroll_group');
	...
	public function del(){
		...
		if($ids){
            ...
            $this->Enroll_model->delItems($ids,'enroll_direct_major');
            $this->Enroll_model->delItems($ids,'enroll_direct_form');
